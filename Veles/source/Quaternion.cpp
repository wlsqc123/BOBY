...
Quaternion Quaternion::MultyFloat(const Quaternion& p, float factor)
{
	XMFLOAT4 rat(p.x * factor, p.y * factor, p.z * factor, p.w * factor);
	return Quaternion(rat);
}

Quaternion Quaternion::AddQuaternion(const Quaternion& p, const Quaternion& b)
{
	XMFLOAT4 rat(p.x + b.x, p.y + b.y, p.z + b.z, p.w + b.w);
	return Quaternion(rat);
}
