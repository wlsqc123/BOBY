...
public:
	...
	Matrix4x4(const aiMatrix4x4& aimatrix);

	Matrix4x4 operator*(const Quaternion& other) const;
	Matrix4x4 operator*(const DirectX::XMFLOAT4X4& other) const;
	Matrix4x4 operator*(const float& other) const;

	Matrix4x4& operator*=(const Quaternion& other);
	Matrix4x4& operator*=(const DirectX::XMFLOAT4X4& other);
	Matrix4x4& operator*=(const float& other);

	Matrix4x4 operator+(const DirectX::XMFLOAT4X4& other) const;
	Matrix4x4& operator+=(const DirectX::XMFLOAT4X4& other);

	bool operator==(const DirectX::XMFLOAT4X4& other) const;
	bool operator!=(const DirectX::XMFLOAT4X4& other) const;
